...
const MenuAdd = () => {
    ...
    const { category, contents,menus, message, nameErr, pageErr, danhmucErr, danhmucchaErr } = useSelector(state => state.app)
    useEffect(() => {
    ...
    const handleSubmit  = (e) => {
        e.preventDefault();
        dispatch(actions.addMenu(formData))
    }
    const navigate = useNavigate();
    useEffect(() => {
        if(message === "Thêm menu thành công"){
            navigate("/menu")
        }
    }, [message, navigate])
    return (
        <div className="full pt-3 sm:pt-5">
            <div className="w-full px-4 sm:px-6 md:px-[30px] flex gap-4 sm:gap-8">
                <div className="w-full">
                    ...
                    <h2 className="text-xl sm:text-2xl md:text-[35px] font-semibold mt-3 sm:mt-0">Thêm mới menu</h2>
                </div>
            </div>
