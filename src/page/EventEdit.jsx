...
const EventEdit = () => {
    ...
    const { event, eventEdit, message, tendanhmucErr, slugErr } = useSelector((state) => state.app);

    useEffect(() => {
        dispatch(actions.getEvent());
        dispatch(actions.getEventEdit(id));
    }, [dispatch, id]);
    ...
    useEffect(() => {
        if (eventEdit) {
            setFormData({
                tendanhmuc: eventEdit.tendanhmuc || "",
                slug: eventEdit.slug || "",
                danhmuccha: eventEdit.danhmuccha || "",
                image: eventEdit.image || "",
                mota: eventEdit.mota || "",
                metatitle: eventEdit.metatitle || "",
                metakeywords: eventEdit.metakeywords || "",
                metadescription: eventEdit.metadescription || "",
                sapxep: eventEdit.sapxep || "",
            });
        }
    }, [eventEdit]);
            ...
            <div className="w-full px-4 sm:px-6 md:px-[30px] flex gap-4 sm:gap-8">
                <div className="w-full">
                    <div className="flex items-center gap-1 sm:gap-2 text-xs sm:text-sm md:text-[15px] text-color">
                        ...
                        <MdChevronRight className="text-sm sm:text-base" />
                        <NavLink to={`/event/${id}`} className={"text-blue-600"}>
                            Chỉnh sửa sự kiện
                        </NavLink>
                    </div>
