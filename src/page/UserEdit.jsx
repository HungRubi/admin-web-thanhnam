...
const UserEdit = () => {
    ...
    useEffect(() => {
        dispatch(actions.getUserEdit(id));
    }, [id, dispatch])
    const { message, accountErr, emailErr, passwordErr, nameErr, userEdit} = useSelector(state => state.app)
    ...
    useEffect(() => {
        if(userEdit) {
            setFormData({
                hovaten: userEdit?.hovaten || '',
                tendangnhap: userEdit?.tendangnhap || '',
                email: userEdit?.email || '',
                matkhau: userEdit?.matkhau || '',
                confirm: userEdit?.matkhau || '',
    ...
    useEffect(() => {
        if(message === "Cập nhật user thành công :))"){
            navigate("/user")
        }
    }, [message, navigate])
    return (
